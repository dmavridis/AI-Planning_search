...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        actions = self.problem.actions_list
        ...
        for node in self.s_levels[level]:
            if node.is_pos:
                node_pos.add(node.symbol)
                noop_action = Action(expr("Noop_pos({})".format(node.symbol)), ([node.symbol], []), ([node.symbol], []))
            else:
                node_neg.add(node.symbol)
                noop_action = Action(expr("Noop_neg({})".format(node.symbol)), ([], [node.symbol]), ([], [node.symbol]))
                
            pg_node = PgNode_a(noop_action)
            pg_node.parents.add(node)
            pg_node.children.add(node)
            self.a_levels[level].add(pg_node)
        
        # Actual action added
        for action in actions:
            if (set(action.precond_pos) <= node_pos) and (set(action.precond_neg) <= node_neg):
                pg_node = PgNode_a(action)
                pg_node.parents = pg_node.prenodes
                pg_node.children = pg_node.effnodes
                self.a_levels[level].add(pg_node)
    ...
    def add_literal_level(self, level):
        ...
        for node in self.a_levels[level - 1]:
            # States originating from Noop Positive or negative
            if node.action.name == 'Noop_pos':
                pg_node = PgNode_s(node.action.args[0], True)
                pg_node.parents.add(node)
                self.s_levels[level].add(pg_node)
                
            elif node.action.name == 'Noop_neg':
                pg_node = PgNode_s(node.action.args[0], False)
                pg_node.parents.add(node)
                self.s_levels[level].add(pg_node)
            else:
              # States originating as effects of actual actions 
                for effect in node.action.effect_add:
                    pg_node = PgNode_s(effect, True)
                    pg_node.parents.add(node)
                    self.s_levels[level].add(pg_node)
                for effect in node.action.effect_rem:
                    pg_node = PgNode_s(effect, False)
                    pg_node.parents.add(node)
                    self.s_levels[level].add(pg_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        ''' 
        # Done
        for s_node_a1 in node_a1.effect_s_nodes():
            for s_node_a2 in node_a2.effect_s_nodes():
                if (s_node_a1.symbol == s_node_a2.symbol) and (s_node_a1.is_pos != s_node_a2.is_pos):
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        for s_node_a1 in node_a1.precond_s_nodes():
            for s_node_a2 in node_a2.effect_s_nodes():
                if (s_node_a1.symbol == s_node_a2.symbol) and (s_node_a1.is_pos != s_node_a2.is_pos):
                    return True
        for s_node_a1 in node_a1.effect_s_nodes():
            for s_node_a2 in node_a2.precond_s_nodes():
                if (s_node_a1.symbol == s_node_a2.symbol) and (s_node_a1.is_pos != s_node_a2.is_pos):
                    return True
        # Done
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        '''
        level_sum = 0
        goal_list = set(self.problem.goal)
        # TODO implement
        # for each goal in the problem, determine the level cost, then add them together
        
        level = 0
        while (level < len(self.s_levels)-1) or goal_list != set():
            for node in self.s_levels[level]:
                if node.literal in goal_list:
                    goal_list.remove(node.literal)
                    level_sum += level
            level += 1
